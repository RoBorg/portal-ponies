...
public class PortalManager : MonoBehaviour
{
    ...
    public class ViewSettingsClass
    {
        ...
        public class DistorsionClass
        {
            ...
            [Range(0, 0.5f)] public float Strength = 0.01f;
        }
        ...
        else
        {
            ...
            else
            {
                ...
                if (PortalMaterials.Length > 0)
                {
                    ...
                    SceneviewRender.GetComponent<MeshRenderer>().sharedMaterial.SetTexture("_MainTex", InGameCamera && PairedPortal.GetComponent<PortalManager>().RenderTexts[0] ? PairedPortal.GetComponent<PortalManager>().RenderTexts[0] : null);
                }
                ...
                SceneviewRender.GetComponent<MeshRenderer>().enabled = PortalSettings.EnableRender ? true : false;
                //Manage distorstion pattern settings
                SceneviewRender.GetComponent<MeshRenderer>().sharedMaterial.SetInt("_EnableDistorsionPattern", ViewSettings.Distorsion.EnableDistorsion ? 1 : 0);
                SceneviewRender.GetComponent<MeshRenderer>().sharedMaterial.SetTexture("_DistorsionPattern", ViewSettings.Distorsion.Pattern);
                SceneviewRender.GetComponent<MeshRenderer>().sharedMaterial.SetColor("_DistorsionPatternColor", ViewSettings.Distorsion.Color);
                SceneviewRender.GetComponent<MeshRenderer>().sharedMaterial.SetInt("_DistorsionPatternTiling", ViewSettings.Distorsion.Tiling);
                SceneviewRender.GetComponent<MeshRenderer>().sharedMaterial.SetFloat("_DistorsionPatternSpeedX", -ViewSettings.Distorsion.SpeedX);
                SceneviewRender.GetComponent<MeshRenderer>().sharedMaterial.SetFloat("_DistorsionPatternSpeedY", -ViewSettings.Distorsion.SpeedY);
                SceneviewRender.GetComponent<MeshRenderer>().sharedMaterial.SetFloat("_DistortionStrength", -ViewSettings.Distorsion.Strength);
                            ...
                            if (RenderTexts[i])
                            {
                                for (int j = ViewSettings.Recursion.Steps; j >= 0; j--)
                                {
                                    if (PortalCamObjs[j])
                                    {
                                        ...
                                        // Camera position relative to paired portal
    ...
    void OnTriggerExit(Collider collision)
    {
        ...
        for (int i = 0; i < CloneCollidedObjs.Length; i++)
        {
            if (ContinueTriggerEvents[i] && CollidedObjs[i] && CollidedObjs[i].GetHashCode().ToString() == collision.gameObject.GetHashCode().ToString() && CloneCollidedObjs[i])
            {
                ...
                GameObject[] CollidedObjsInternalParent = new GameObject[0];//CollidedObjs[i].transform.childCount > 0 ? CollidedObjs[i].transform.childCount : 0];
